...
beforeAll((done: jest.DoneCallback) => {
  ...
  request(app)
    .post("/auth/signup")
    .send({
      email: `comment.test.${unique}@gmail.com`,
    ...
    .end(async (err, res) => {
      ...
      try {
        const commentCount = await prisma.comment.count({
          where: { userId, postId, content: validContent },
        });
  ...
  request(app)
    ...
    .expect((res) => {
      ...
      const contentError = res.body.errors.find(
        (e: any) => e.path === "content",
      );
      expect(contentError).toBeDefined();
      expect(contentError.msg).toBe(
        "Comment must be between 1 and 60 characters",
      );
    })
    .end(done);
});
