...
export default function Post() {
  ...
  const [post, setPost] = useState<PostWithExtras>({
    id: "",
    title: "",
    content: "",
    userId: "",
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const fetched = await api.get<PostWithExtras>(`/post/${postId}`);
  ...
  async function deleteComment(id: string) {
    ...
    try {
      ...
      setComments((prev) => {
        ...
        setVisible((vc) => Math.min(vc, next.length));
        return next;
      });
    } catch (err) {
  ...
  const loadMoreHeight = visible < comments.length ? 36 : 0;
  const extraSpace = inputAreaHeight + loadMoreHeight + 16; // padding
  const computedMaxHeight = showComments
    ? Math.max(400, visible * commentItemHeight + extraSpace)
    : 0;
